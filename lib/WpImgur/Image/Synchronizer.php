...
class Synchronizer {
  ...
  function sync($id) {
    ...
    if (count($images) > 0) {
      ...
      $imageStore = $this->imageStoreFor($slug, $id);
  ...
  function syncImage($image, $imageStore) {
    ...
    if ($size == "original" && !$imageStore->hasImage($size)) {
      return $this->uploadAndSave($image, $imageStore);
    } elseif ($size == "original" && !$this->imageUrlExists($imageStore->getImageUrl($size))) {
      return $this->uploadAndSave($image, $imageStore);
    } else {
  ...
  function imageStoreFor($slug, $id) {
